  ...
  const typeFilterHandler = event => {
    ...
    if(titleFilter.length === 0) {
      if(event.target.value === 'default') {
        ...
        filteredProducts = products.filter(product => product.type === event.target.value)
      }
    } else {
      if(event.target.value === 'default') {
        filteredProducts = products.filter(product => product.title.toLowerCase().includes(titleFilter.toLowerCase()))
      } else {
        filteredProducts = products.filter(product => product.type === event.target.value && product.title.toLowerCase().includes(titleFilter.toLowerCase()))
      }
    }
    setAds(paginateArray(filteredProducts, 4))
  }
  ...
  return (
    <div className={classes.ads}>
        ...
        <div className={classes.products}>
          ...
          <ul className={classes.pagination}>
            {Object.values(ads).map((_,adIndex) => {
              return (
                <li key={adIndex} className={index === adIndex ? classes.active : undefined } onClick={() => {paginationHandler(adIndex)}}>{adIndex + 1}</li>
              )
            })}
          </ul>
        </div>
      ...
      <div className={classes.filter}>
        <h2>Filters</h2>
        <input type="text" placeholder='Title' value={titleFilter} onChange={titleFilterHandler} />
